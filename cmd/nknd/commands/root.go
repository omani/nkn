...
func init() {
	...
	rootCmd.PersistentFlags().BoolVarP(&createMode, "create", "c", false, "Create Mode")
	rootCmd.PersistentFlags().StringVar(&config.SeedList, "seed", "", "Seed node address to join, multiple seeds should be split by comma")
	rootCmd.PersistentFlags().StringVarP(&password.Passwd, "passwd", "p", "", "Password of Your wallet private Key")
	rootCmd.PersistentFlags().BoolVar(&config.SkipNAT, "no-nat", false, "Skip NAT traversal for UPnP and NAT-PMP")
	rootCmd.PersistentFlags().BoolVar(&config.Debug, "debug", false, "Provide runtime profiling data of NKN")
	rootCmd.PersistentFlags().StringVar(&config.StatePruningMode, "pruning", "", "state pruning mode: none, lowmem")
	rootCmd.PersistentFlags().StringVar(&config.SyncMode, "sync", "", "sync mode: full, fast, light")
	rootCmd.PersistentFlags().StringVar(&config.PprofPort, "pprof-port", "", "The port used for pprof in debug mode")
	rootCmd.PersistentFlags().StringVar(&config.ConfigFile, "config", "", "config file name")
	rootCmd.PersistentFlags().StringVar(&config.LogPath, "log", "", "directory where your log file will be generated")
	rootCmd.PersistentFlags().StringVar(&config.ChainDBPath, "chaindb", "", "directory where your blockchain data will be stored")
	rootCmd.PersistentFlags().StringVar(&config.WalletFile, "wallet", "", "wallet file")
	rootCmd.PersistentFlags().StringVar(&config.BeneficiaryAddr, "beneficiaryaddr", "", "beneficiary address where your mining reward will go to")
	rootCmd.PersistentFlags().StringVar(&config.GenesisBlockProposer, "genesisblockproposer", "", "public key of genesis block proposer")
	rootCmd.PersistentFlags().BoolVar(&config.AllowEmptyBeneficiaryAddress, "allow-empty-beneficiary-address", false, "beneficiary address is forced unless --allow-empty-beneficiary-address is true")
	rootCmd.PersistentFlags().StringVar(&config.WebGuiListenAddress, "web-gui-listen-address", "", "web gui will listen this address (default: 127.0.0.1)")
	rootCmd.PersistentFlags().BoolVar(&config.WebGuiCreateWallet, "web-gui-create-wallet", false, "web gui create/open wallet")
	rootCmd.PersistentFlags().StringVar(&config.PasswordFile, "password-file", "", "read password from file, save password to file when --web-gui-create-wallet arguments be true and password file does not exist")
}
